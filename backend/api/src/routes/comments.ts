...
router.post('/create', auth, async (req, res) => {
  ...
  const currentProfileObject = await prisma.users.findUnique({
    where: {
      id: userId,
    },
    select: {
  ...
  const currentProfileId = currentProfileObject.currentProfileId;

  console.log('currentProfileId: ', currentProfileId)

  const userNameObject = await prisma.profiles.findUnique({
    where: {
      id: currentProfileId,
    },
    select: {
